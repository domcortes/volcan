...
$ARRAYEXISTENCIA = $EXIMATERIAPRIMA_ADO->buscarPorFolioEmpresaPlantaTemporada($NUMEROFOLIO, $EMPRESA , $PLANTA , $TEMPORADA );
...
foreach ($ARRAYEXISTENCIA as $s) :
	...
	$NOMBREPRODUCTOR=$ARRAYPRODUCTOR[0]['NOMBRE_CORTADO'];
	$CSGPRODUCTOR=$ARRAYPRODUCTOR[0]['CSG_PRODUCTOR'];

	if($s['ID_RECEPCION'] !=NULL && $s['ID_REPALETIZAJE'] == NULL){
		$ARRAYRECEPCION = $RECEPCION_ADO->verRecepcion($s['ID_RECEPCION'] );
		$ARRAYTIPO=$TRECEPCION_ADO->verTrecepcion($ARRAYRECEPCION[0]['ID_TRECEPCION']);
		$TIPO=$ARRAYTIPO[0]['NOMBRE_TRECEPCION'];

	}
	if( $s['ID_REPALETIZAJE'] != NULL){
		$ARRAYREPALETIZAJE = $REPALETIZAJEMP_ADO->verRepaletizaje2($s['ID_REPALETIZAJE']);
		$TIPO="Materia Prima";
...
$html=$html.'
		<div class="info">
			<b> Numero Recepcion : </b> '.$ARRAYRECEPCION[0]['NUMERO_RECEPCION'].'
		</div>
		<div class="info">
			<b> Numero guia : </b> '.$ARRAYRECEPCION[0]['NUMERO_GUIA_RECEPCION'].'
		</div>
		<div class="info">
			<b> Fecha Recepcion : </b>  '.$ARRAYRECEPCION[0]['FECHA_RECEPCIONR'].'
		</div>
		';
}
...
if( $s['ID_REPALETIZAJE'] != NULL){
	$html=$html.'
	<div class="info">
		<b> Numero Repaletizaje : </b> '.$ARRAYREPALETIZAJE[0]['NUMERO_REPALETIZAJE'].'
	</div>
	<div class="info">
		<b> Fecha Repaletizaje : </b>  '.$ARRAYREPALETIZAJE[0]['INGRESO'].'
	</div>	
...
if($s['ID_RECEPCION'] !=NULL && $s['ID_REPALETIZAJE'] == NULL){	
		...
		$ARRAYDRECEPCION = $DRECEPCION_ADO->buscarPorRecepcionFolio2($s['FOLIO_AUXILIAR_EXIMATERIAPRIMA']); 
		$html=$html.'
		<div class="info justify">
			<b> Kilos Brutos : </b>  '.$ARRAYDRECEPCION[0]['BRUTO'].'
		</div>
';
}
...
$html=$html.'
        ...
        <div class="f30 center">
            <b> '.$s['FOLIO_AUXILIAR_EXIMATERIAPRIMA'].' </b> 
        </div>
        <br>
        <div class=" center">
             Variedad  
        ...
        <div class="subtitulo center" style="font-size: 18px; text-align: center;">
			 <barcode code="'.$s['ALIAS_FOLIO_EXIMATERIAPRIMA'].'" size="0.9" type="QR"  class="barcode" disableborder="1" />
		</div>
