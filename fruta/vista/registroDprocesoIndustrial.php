...
if (isset($_REQUEST['GUARDAR'])) {
    ...
    $ARRAYOBTENERNUMEROLINEA = $DPINDUSTRIAL_ADO->obtenerNumeroLinea($_REQUEST['IDPROCESO']);
    ...
    if ($ARRAYULTIMOFOLIO[0]['ULTIMOFOLIO'] == 0) {
        ...
        $FOLIODPINDUSTRIAL = $ARRAYVERFOLIO[0]['NUMERO_FOLIO'] + $ARRAYULTIMOFOLIO[0]['ULTIMOFOLIO'];     
    ...
    $FOLIOALIAS =  "EMPRESA:".$_REQUEST['EMPRESA']."_PLANTA:".$_REQUEST['PLANTA']."_TEMPORADA:".$_REQUEST['TEMPORADA']."_TIPO_FOLIO:PRODUCTO INDUSTRIAL_NUMEROLINEA:".$NUMEROLINEA."PROCESO:".$_REQUEST['IDPROCESO'];       
    $ARRAYVERFOLIOPOIND = $EXIINDUSTRIAL_ADO->buscarPorProcesoNumeroLinea($_REQUEST['IDPROCESO'], $NUMEROLINEA , $NUMEROFOLIODINDUSTRIAL);
 
 
    $DPINDUSTRIAL->__SET('FOLIO_DPINDUSTRIAL', $NUMEROFOLIODINDUSTRIAL);
    $DPINDUSTRIAL->__SET('NUMERO_LINEA', $NUMEROLINEA);
    $DPINDUSTRIAL->__SET('FOLIO_AUX_DPINDUSTRIAL', $ULTIMOFOLIO);
    $DPINDUSTRIAL->__SET('FECHA_EMBALADO_DPINDUSTRIAL', $_REQUEST['FECHAEMBALADODINDUSTRIAL']);
    $DPINDUSTRIAL->__SET('KILOS_NETO_DPINDUSTRIAL', $KILOSNETO);
    $DPINDUSTRIAL->__SET('ALIAS_FOLIO_DPINDUSTRIAL', $FOLIOALIAS);
    $DPINDUSTRIAL->__SET('ID_ESTANDAR', $_REQUEST['EEXPORTACION']);
    $DPINDUSTRIAL->__SET('ID_FOLIO', $FOLIO); 
    $DPINDUSTRIAL->__SET('ID_PVESPECIES',  $_REQUEST['PVESPECIES']); 
    $DPINDUSTRIAL->__SET('ID_PROCESO', $_REQUEST['IDPROCESO']);
    ...
    if(empty($ARRAYVERFOLIOPOIND)){       
        ...
        $EXIINDUSTRIAL->__SET('ID_PROCESO', $_REQUEST['IDPROCESO']);
...
if (isset($_REQUEST['EDITAR'])) {
    //OBTENER EL FOLIO DEL DETALLE DE EXPORTACION DEL PROCESO
    $ARRAYDPROCESOINDUSTRIAL2 = $DPINDUSTRIAL_ADO->verDpindustrial($_REQUEST['IDDPROCESOINDUSTRIAL']);
    $ARRAYVERFOLIO = $FOLIO_ADO->verFolioPorEmpresaPlantaTemporadaTindustrial($_REQUEST['EMPRESA'],$_REQUEST['PLANTA'],$_REQUEST['TEMPORADA']);    
    
    $FOLIO=$ARRAYVERFOLIO[0]['ID_FOLIO'];
  
    $NUMEROLINEA = ($ARRAYDPROCESOINDUSTRIAL2[0]['NUMERO_LINEA']);
    $NUMEROFOLIODINDUSTRIAL = $ARRAYVERFOLIO[0]['NUMERO_FOLIO']+$ARRAYDPROCESOINDUSTRIAL2[0]['FOLIO_AUX_DPINDUSTRIAL'];   
    ...
    $DPINDUSTRIAL->__SET('ID_PROCESO', $_REQUEST['IDPROCESO']);
    $DPINDUSTRIAL->__SET('ID_PRODUCTOR', $_REQUEST['PRODUCTOR']);
    $DPINDUSTRIAL->__SET('ID_DPINDUSTRIAL', $_REQUEST['IDDPROCESOINDUSTRIAL']);
    //LLAMADA AL METODO DE REGISTRO DEL CONTROLADOR
...
if (isset($_REQUEST['IDDPROCESOINDUSTRIAL'])   && isset($_REQUEST['PRODUCTOR'])&& isset($_REQUEST['PVESPECIES']) && isset($_REQUEST['EMPRESA']) && isset($_REQUEST['PLANTA']) && isset($_REQUEST['TEMPORADA'])&& isset($_REQUEST['OP'])) {
    ...
    if ($OP == "crear") {
        ...
        $DISABLED2 = "disabled";
        $DISABLEDSTYLE2 = "style='background-color: #eeeeee;'";
        $ARRAYDPROCESOINDUSTRIAL=$DPINDUSTRIAL_ADO->verDpindustrial($IDDPROCESOINDUSTRIAL);
        foreach ($ARRAYDPROCESOINDUSTRIAL as $r) :
            ...
            $CANTIDADENVASEDINDUSTRIAL="" . $r['KILOS_NETO_DPINDUSTRIAL'];
