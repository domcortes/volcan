...
if (isset($_REQUEST['GUARDAR'])) {
    ...
    $ARRAYOBTENERNUMEROLINEA = $DPEXPORTACION_ADO->obtenerNumeroLinea($_REQUEST['IDPROCESO']);
    ...
    //$ARRAYULTIMOFOLIO = $DPEXPORTACION_ADO->obtenerFolio($FOLIO);    
    ...
    //$PDESHIDRATACIONEESTANDAR=$_REQUEST['PDESHIDRATACIONDPEXPORTACION'];
    ...
    $FOLIOALIAS =  "EMPRESA:" . $_REQUEST['EMPRESA'] . "_PLANTA:" . $_REQUEST['PLANTA'] . "_TEMPORADA:" . $_REQUEST['TEMPORADA'] . "_TIPO_FOLIO:PRODUCTO TERMINADO_NUMEROLINEA:" . $NUMEROLINEA . "PROCESO:" . $_REQUEST['IDPROCESO'];
    $ARRAYVERFOLIOPOEXPO = $EXIEXPORTACION_ADO->buscarPorProcesoNumeroLinea($_REQUEST['IDPROCESO'], $NUMEROLINEA, $NUMEROFOLIODEXPORTACION);

    $DPEXPORTACION->__SET('FOLIO_DPEXPORTACION', $NUMEROFOLIODEXPORTACION);
    $DPEXPORTACION->__SET('NUMERO_LINEA', $NUMEROLINEA);
    $DPEXPORTACION->__SET('FOLIO_AUX_DPEXPORTACION', $NUMEROFOLIODEXPORTACION);
    $DPEXPORTACION->__SET('FECHA_EMBALADO_DPEXPORTACION', $_REQUEST['FECHAEMBALADODEXPORTACION']);
    $DPEXPORTACION->__SET('CANTIDAD_ENVASE_DPEXPORTACION', $_REQUEST['CANTIDADENVASEDEXPORTACION']);
    $DPEXPORTACION->__SET('KILOS_NETO_DPEXPORTACION', $KILOSNETO);
    $DPEXPORTACION->__SET('KILOS_BRUTO_DPEXPORTACION', $KILOSBRUTO);
    ...
    $DPEXPORTACION->__SET('EMBOLSADO', $EMBOLSADO);
    $DPEXPORTACION->__SET('ALIAS_FOLIO_DPEXPORTACION', $FOLIOALIAS);
    $DPEXPORTACION->__SET('ID_ESTANDAR', $_REQUEST['EEXPORTACION']);
    $DPEXPORTACION->__SET('ID_CALIBRE', $_REQUEST['CALIBRE']);
    $DPEXPORTACION->__SET('ID_FOLIO', $FOLIO);
    $DPEXPORTACION->__SET('ID_PVESPECIES',  $_REQUEST['PVESPECIES']);
    $DPEXPORTACION->__SET('ID_PRODUCTOR', $_REQUEST['PRODUCTOR']);
    $DPEXPORTACION->__SET('ID_PROCESO', $_REQUEST['IDPROCESO']);
    ...
    if (empty($ARRAYVERFOLIOPOEXPO)) {
        ...
        $EXIEXPORTACION->__SET('ID_PROCESO', $_REQUEST['IDPROCESO']);
...
if (isset($_REQUEST['EDITAR'])) {
    ...
    //OBTENER EL FOLIO DEL DETALLE DE EXPORTACION DEL PROCESO
    $ARRAYDPROCESOEXPORTACION2 = $DPEXPORTACION_ADO->verDpexportacion($_REQUEST['IDDPROCESOEXPORTACION']);
    $ARRAYVERFOLIO = $FOLIO_ADO->verFolioPorEmpresaPlantaTemporadaTexportacion($_REQUEST['EMPRESA'], $_REQUEST['PLANTA'], $_REQUEST['TEMPORADA']);
    $FOLIO = $ARRAYVERFOLIO[0]['ID_FOLIO'];

    $NUMEROLINEA = ($ARRAYDPROCESOEXPORTACION2[0]['NUMERO_LINEA']);
    $NUMEROFOLIODEXPORTACION = $ARRAYDPROCESOEXPORTACION2[0]['FOLIO_AUX_DPEXPORTACION'];
    ...
    $DPEXPORTACION->__SET('ID_PRODUCTOR', $_REQUEST['PRODUCTOR']);
    $DPEXPORTACION->__SET('ID_TMANEJO', $_REQUEST['TMANEJO']);
    $DPEXPORTACION->__SET('ID_DPEXPORTACION', $_REQUEST['IDDPROCESOEXPORTACION']);
    //LLAMADA AL METODO DE REGISTRO DEL CONTROLADOR
...
if (isset($_REQUEST['IDPROCESO'])  && isset($_REQUEST['PRODUCTOR']) && isset($_REQUEST['PVESPECIES']) && isset($_REQUEST['EMPRESA']) && isset($_REQUEST['PLANTA']) && isset($_REQUEST['TEMPORADA'])) {
    ...
    $ARRAYESTANDAR = $EEXPORTACION_ADO->buscarVespeciesPorEspecies($ARRAYVESPECIES[0]['ID_ESPECIES']);
...
if (isset($_REQUEST['IDDPROCESOEXPORTACION']) &&  isset($_REQUEST['PRODUCTOR']) && isset($_REQUEST['PVESPECIES']) && isset($_REQUEST['EMPRESA']) && isset($_REQUEST['PLANTA']) && isset($_REQUEST['TEMPORADA']) && isset($_REQUEST['OP'])) {
    ...
    if ($OP == "crear") {
        ...
        $DISABLEDSTYLE = "";
        $DISABLEDSTYLE2 = "style='background-color: #eeeeee;'";
        $ARRAYDPROCESOEXPORTACION = $DPEXPORTACION_ADO->verDpexportacion($IDDPROCESOEXPORTACION);
        foreach ($ARRAYDPROCESOEXPORTACION as $r) :

            $NUMEROFOLIODEXPORTACION = "" . $r['FOLIO_DPEXPORTACION'];
            $FECHAEMBALADODEXPORTACION = "" . $r['FECHA_EMBALADO_DPEXPORTACION'];
            $CANTIDADENVASEDEXPORTACION = "" . $r['CANTIDAD_ENVASE_DPEXPORTACION'];
            $PDESHIDRATACIONDPEXPORTACION = "" . $r['PDESHIDRATACION_DPEXPORTACION'];
...
if ($_POST) {
    ...
    $PDESHIDRATACIONDPEXPORTACION = $PDESHIDRATACIONEESTANDAR;
