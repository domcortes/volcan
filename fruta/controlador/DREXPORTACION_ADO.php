 ...
 //LISTAR TODO CON LIMITE DE 6 FILAS   
    ...
    public function agregarDrexportacion(DREXPORTACION $DREXPORTACION){
        try{
            ...
            $query=
            "INSERT INTO `fruta_drexportacion` (`FOLIO_DREXPORTACION`,
                                          `NUMERO_LINEA`,`FOLIO_AUX_DREXPORTACION`,
                                          `FECHA_EMBALADO_DREXPORTACION`,
                                          `CANTIDAD_ENVASE_DREXPORTACION`,`KILOS_NETO_DREXPORTACION`,`KILOS_BRUTO_DREXPORTACION`,`KILOS_DESHIDRATACION_DREXPORTACION`,
                                          `PDESHIDRATACION_DREXPORTACION`, `EMBOLSADO`,
                                          `ALIAS_FOLIO_DREXPORTACION`,
                                          `ID_ESTANDAR`, `ID_CALIBRE`,`ID_FOLIO`, `ID_PVESPECIES`,`ID_REEMBALAJE`,`ID_PRODUCTOR`,`ID_TMANEJO`,
            ...
            ->execute(
                array(
                    ...
                    $DREXPORTACION->__GET('NUMERO_LINEA'),
                    ...
                    $DREXPORTACION->__GET('ALIAS_FOLIO_DREXPORTACION'),
                    $DREXPORTACION->__GET('ID_ESTANDAR'),
                    $DREXPORTACION->__GET('ID_CALIBRE'),
                    $DREXPORTACION->__GET('ID_FOLIO'),
                    $DREXPORTACION->__GET('ID_PVESPECIES'),
    ...
    public function eliminarDrexportacion($id){
        try{$sql="DELETE FROM `fruta_drexportacion` WHERE `ID_DREXPORTACION`=".$id.";";
...
public function buscarPorReembalaje2($IDREEMBALAJE){
    try{
        ...
        $datos=$this->conexion->prepare("SELECT *,  DATE_FORMAT(FECHA_EMBALADO_DREXPORTACION, '%d-%m-%Y') AS 'FECHA',
...
public function obtenerTotales($IDPROCESO){
    try{
        ...
        $datos=$this->conexion->prepare("SELECT IFNULL(SUM(CANTIDAD_ENVASE_DREXPORTACION),0) AS 'TOTAL_ENVASE', 
                                                IFNULL(SUM(KILOS_NETO_DREXPORTACION),0) AS 'TOTAL_NETO' , 
                                                IFNULL(SUM(KILOS_BRUTO_DREXPORTACION),0) AS 'TOTAL_BRUTO' , 
                                                IFNULL(SUM(KILOS_DESHIDRATACION_DREXPORTACION),0) AS 'TOTAL_DESHIDRATACION' 
                                         FROM `fruta_drexportacion` 
...
public function obtenerTotales2($IDPROCESO){
    try{
        ...
        $datos=$this->conexion->prepare("SELECT FORMAT(IFNULL(SUM(CANTIDAD_ENVASE_DREXPORTACION),0),0,'de_DE') AS 'TOTAL_ENVASE', 
                                                FORMAT(IFNULL(SUM(KILOS_NETO_DREXPORTACION),0),2,'de_DE') AS 'TOTAL_NETO' , 
                                                FORMAT(IFNULL(SUM(KILOS_BRUTO_DREXPORTACION),0),2,'de_DE') AS 'TOTAL_BRUTO' , 
                                                FORMAT(IFNULL(SUM(KILOS_DESHIDRATACION_DREXPORTACION),0),2,'de_DE') AS 'TOTAL_DESHIDRATACION' 
                                         FROM `fruta_drexportacion` 
                                         WHERE ID_REEMBALAJE = '".$IDPROCESO."' 
                                         AND `ESTADO_REGISTRO` = 1;");
        $datos->execute();
