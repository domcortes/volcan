...
class EXIMATERIAPRIMA_ADO
{
    ...
    public function buscarPorRecepcion($IDRECEPCION)
    {
        try {
            ...
            $datos = $this->conexion->prepare("SELECT * FROM fruta_eximateriaprima WHERE ID_RECEPCION= '" . $IDRECEPCION . "'  AND ESTADO_REGISTRO = 1;");
            $datos->execute();
            $resultado = $datos->fetchAll();
    ...
    public function listarEximateriaprimaEmpresaPlantaTemporadaDisponible($EMPRESA,  $PLANTA, $TEMPORADA)
    {
        try {
            ...
            $datos = $this->conexion->prepare("SELECT *,  
                                                    ...
                                                    IFNULL(DATE_FORMAT(INGRESO, '%d-%m-%Y'),'Sin Datos') AS 'INGRESOF',
                                                    IFNULL(DATE_FORMAT(MODIFICACION, '%d-%m-%Y'),'Sin Datos') AS 'MODIFICACIONF',
